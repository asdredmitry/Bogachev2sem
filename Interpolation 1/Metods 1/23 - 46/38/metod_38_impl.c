...
void Coeff_38(int n, double *x, double *f_x, double *c, double *d, double *a1, double *c1, double *d1)
{
	int i, j;
	double tmp1, tmp2;
	...
	tmp2 = (f_x[2] - f_x[1]) / (x[2] - x[1]);

	c[0] = (tmp1 * (x[2] - x[1]) * (2.0 * x[2] + x[1] - 3.0 * x[0]) + tmp2 * (x[1] - x[0]) * (x[1] - x[0])) / (x[2] - x[0]);

	tmp1 = (f_x[n - 2] - f_x[n - 3]) / (x[n - 2] - x[n - 3]);
	tmp2 = (f_x[n - 1] - f_x[n - 2]) / (x[n - 1] - x[n - 2]);
	c[n - 1] = (tmp1 * (x[n - 1] - x[n - 2]) * (x[n - 1] - x[n - 2]) + tmp2 * (x[n - 2] - x[n - 3]) * (3.0 * x[n - 1] - x[n - 2] - 2.0 * x[n - 3])) / (x[n - 1] - x[n - 3]);
...
double Value_38(double t, int n, double *c, double *x)
{
	int i;
