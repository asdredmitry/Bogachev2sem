...
void Coeff_42(int n, double a, double b, double *x, double *f_x, double *c, double *v, double *ksi, double *a1, double *c1, double *d1)
{
	int i, j;
	double tmp1;
	...
	a1[0] = 2.0/((x[0] - ksi[0]) * (ksi[1] - ksi[0]));
	c1[0] = 2.0/((ksi[1] - x[0]) * (ksi[1] - ksi[0]));
	a1[n] = 2.0/((ksi[n] - x[n - 1]) * (ksi[n] - ksi[n - 1]));
	d1[n - 1] = 2.0/((x[n - 1] - ksi[n - 1]) * (ksi[n] - ksi[n - 1]));

	c[0] = f_second(x[0]) + 2.0/(ksi[1] - ksi[0]) * f_x[0] * (1.0/(x[0] - ksi[0]) + 1.0/(ksi[1] - x[0]));
	c[n] = f_second(x[n - 1]) + 2.0/(ksi[n] - ksi[n - 1]) * f_x[n - 1] * (1.0/(x[n - 1] - ksi[n - 1]) + 1.0/(ksi[n] - x[n - 1]));
...
double Value_42(double t, int n, double *c, double *ksi)
{
	int i;
